...
static bool s_ok;
static char s_status_t[96];
static char s_meta_t[96];
static char s_state_t[96];
static sensor_cfg_t s_cfg;

static void slug_room(const char *room, char *out, size_t n)
{
    if (!room || !room[0]) {
        snprintf(out, n, "unassigned");
        return;
    }
    size_t j = 0;
...
void mqtt_sensor_publish_state(float t_c, float rh, int bat, int rssi)
{
    ...
    snprintf(payload, sizeof(payload),
             "{\"type\":\"temp_hum\",\"t\":%.2f,\"h\":%.2f,\"bat\":%d,\"rssi\":%d,\"ts\":%ld}", t_c, rh,
             bat, rssi, (long)time(NULL));
